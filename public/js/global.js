    ...
    socket.on('loggedInUser', function(users){
        ...
        for(var i = 0; i < users.length; i++){
            if(friend.indexOf(users[i].name) > -1){
                ...
                var image = users[i].img ? '/uploads/' + users[i].img : 'https://placehold.it/300x300';

                var list = '<img src="' + image+'" class="pull-left img-circle" style="width:50px; height:50px; margin-right:10px;" /><p>' +
                ...
                '<div class="clearfix"></div><hr style=" margin-top: 5px; margin-bottom: 5px;" />'
                ol.append(list);
            }
        }
